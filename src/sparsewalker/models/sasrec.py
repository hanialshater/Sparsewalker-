...
class FFN(nn.Module):
    def __init__(self, d, inner, dropout, swiglu=False):
        ...
        self.swiglu = swiglu
        if swiglu:
            # LiGR reference: SiLU(W1 x) * W3 x -> dropout -> W2.
    ...
    def forward(self, x):
        if self.swiglu:
            return self.down(self.drop(F.silu(self.up(x)) * self.gate(x)))
        return self.net(x)
...
class SASBlock(nn.Module):
    def __init__(self, d, heads, inner, dropout, ligr=False):
        ...
        self.ffn = FFN(d, inner, dropout, swiglu=ligr)
        self.d1 = nn.Dropout(dropout)
        self.d2 = nn.Dropout(dropout)
        if ligr:
            # Reference LiGR residual gates include bias.
            self.g1 = nn.Linear(d, d, bias=True)
            self.g2 = nn.Linear(d, d, bias=True)
...
class SASRec(ARRecommender):
    def __init__(
        ...
        self.blocks = nn.ModuleList([
            SASBlock(d, heads, inner, dropout, ligr=ligr)
